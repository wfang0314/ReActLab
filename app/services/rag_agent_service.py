...
class RagAgentService:
    ...
    async def query_stream(
        ...
        try:
            ...
            # Persist session to file store
            try:
                history = self.get_session_history(session_id)
                if history:
                    session_store.save(session_id, history)
            except Exception as se:
                logger.warning(f"Failed to persist stream session: {se}")
    ...
    def get_session_history(self, session_id: str) -> list:
        ...
        try:
            ...
            file_history = session_store.load(session_id)
            if file_history:
                logger.info(f"从文件恢复会话历史: {session_id}, 消息数量: {len(file_history)}")
                return file_history
            return []
