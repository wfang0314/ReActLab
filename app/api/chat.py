...
async def chat(request: ChatRequest):
    ...
    try:
        ...
        try:
            history = session_store.load(request.id) or []
            from datetime import datetime
            history.append({"role": "user", "content": request.question, "timestamp": datetime.now().isoformat()})
            history.append({"role": "assistant", "content": answer, "timestamp": datetime.now().isoformat()})
            session_store.save(request.id, history)
        except Exception as se:
            logger.warning(f"Failed to save session: {se}")
    ...
    async def event_generator():
        try:
            ...
            try:
                ...
                logger.warning(f"Failed to save stream session: {se}")
...
async def get_session_info(session_id: str) -> SessionInfoResponse:
    ...
    """
    try:
                # Try file store first, fallback to MemorySaver
